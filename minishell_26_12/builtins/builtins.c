...
/*   Updated: 2023/12/04 20:48:41 by mneves-l         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	choose_builtin(t_data *data)
{
	if (!ft_strncmp(data->cmds->comand[0], "env", 4))
		print_env(&data->env);
	if (!ft_strncmp(data->cmds->comand[0], "pwd", 4))
		do_pwd(&data->env);
	if (!ft_strncmp(data->cmds->comand[0], "echo", 5))
		do_echo(data->cmds);
	if (!ft_strncmp(data->cmds->comand[0], "export", 7))
		do_export(data->env, data->cmds, data);
	if (!ft_strncmp(data->cmds->comand[0], "unset", 6))
		do_unset(data->cmds, &data->env);
	if (!ft_strncmp(data->cmds->comand[0], "cd", 3))
		do_cd(data->env, data->cmds);
	if (!ft_strncmp(data->cmds->comand[0], "exit", 5))
		do_exit(data->cmds);
	//else
		//execute_single_comand(data);
}
